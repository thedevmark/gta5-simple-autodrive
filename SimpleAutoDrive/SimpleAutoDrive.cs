...
// SimpleAutoDrive v4 — the task drives, I don't.
//
// v1-v3 added speed overrides, angle checks, progress monitors, wrong-way
// detection, stall timers, loop breakers, dead-stop resets — every one
// fought the LONGRANGE task's own AI and caused new problems. v4 removes
// ALL of it. The task is the same system Rockstar uses for mission
// chauffeurs and taxi sequences. It knows how to drive.
//
// What I control: destination, speed ceiling, overtaking, the shot.
...
public class SimpleAutoDrive : Script
{
    ...
    // 28 m/s (~100 km/h): the speed mission NPC drivers use. Reliable in
    // parking lots, bridges, tight corners — every environment the AI
    // encounters. Higher speeds cause wall collisions in unpredictable
    // environments regardless of how smart the speed management is.
    const float MaxSafeSpeed = 28.0f;
    ...
    void OnTick(object sender, EventArgs e)
    {
        ...
        // overtake / shoot when sagging behind a slower car
    ...
    void TryOvertake(Ped p, Vehicle v)
    {
        ...
        foreach (Vehicle other2 in World.GetNearbyVehicles(_passTarget, corridorLength + 30.0f))
        {
            ...
            if (lateral < 3.5f) return; // oncoming: wait for a gap
        }
    ...
    void CycleTier()
    {
        ...
        if (_on)
        {
            ...
            Function.Call(Hash.SET_DRIVE_TASK_MAX_CRUISE_SPEED, Game.Player.Character, speed, 1);
    ...
    void Retask()
    {
        ...
        // destination close and behind: short-range task for U-turns
        float distNow = v.Position.DistanceTo(_target);
        Vector3 dir = _target - v.Position; dir.Z = 0f;
        Vector3 fwd = v.ForwardVector; fwd.Z = 0f;
        bool needUTurn = false;
        if (dir.LengthSquared() > 1f && fwd.LengthSquared() > 0.01f && distNow < 150.0f)
        {
        ...
        if (needUTurn)
        {
            Function.Call((Hash)0xE2A2AA2F659D77A7, p, v,
                ...
                speed, 1, 0, _styles[_tier], _stopRange, -1f);
        }
        else
        {
